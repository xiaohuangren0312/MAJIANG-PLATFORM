...
class HistoryBondApiTests(TestCase):
    def test_scoped_access_audit_projection_and_export(self):
        ...
        url=f'/api/events/{event.id}/';body=dict(action='history-bond-register',revision=1,playerId=p['id'],stageId='regular',teamId=p['teamId'])
        self.client.force_login(other);self.assertEqual(self.client.post(url,body,content_type='application/json').status_code,404)
        self.client.force_login(manager);self.assertEqual(self.client.post(url,body,content_type='application/json').status_code,200)
