...
def _public_event(event):
    ...
    for m in sorted(d['matches'],key=lambda m:(m['date'],m['time'],m['number'])):
        ...
        schedule.append(dict(id=m['id'],stageId=m['stageId'],date=m['date'],time=m['time'],table=m['table'],number=m['number'],pairingRound=m.get('pairingRound'),lineupPublished=all(disclosed),seatOrderKnown=bool(m.get("seatDraw") or published),players=[dict(playerId=s['playerId'] if shown else None,teamId=s['teamId'],wind='东南西北'[i],lineupPublished=shown) for i,(s,shown) in enumerate(zip(m['seats'],disclosed))],state='completed' if published else 'cancelled' if m['state']=='cancelled' else 'scheduled',resultId=m['id'] if published else None))
        if published:
