...
async function access(){
 ...
 $('#content').innerHTML=`<h1>账号与赛事授权</h1><p>赛事管理员管理自己的赛事；教练仅有选人大会操作权限；观众只能查询公开内容。</p><div class="card"><h2>赛事管理员</h2><div id="access-members">正在读取…</div></div>${superadmin?`<div class="card"><h2>分配赛事管理员</h2><form id="grant-form"><label>已有账号用户名<input name="username" required maxlength="120"></label><label>授权操作<select name="role"><option value="admin">授予本赛事管理权限</option><option value="revoke">撤回本赛事管理权限</option></select></label><button>保存授权</button></form></div>`:''}${can('coach-manage')||superadmin?`<div class="card"><h2>创建账号</h2><form id="account-form"><label>账号身份<select name="role"><option value="coach">本赛事教练</option>${superadmin?'<option value="admin">本赛事管理员</option><option value="viewer">观众</option>':''}</select></label><label>用户名<input name="username" required maxlength="120" autocomplete="off"></label><label>初始密码<input name="password" type="password" value="1234@qwer" required autocomplete="new-password"></label><button>创建账号</button></form></div>`:''}<div class="card"><h2>本赛事教练账号</h2><div id="coach-accounts">正在读取…</div></div>`;
