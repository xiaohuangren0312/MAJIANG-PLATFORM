...
def asset(request,name):
    ...
    require_names.update({'history-backfill.js','event-ink.png','matchday-landscape.png','team-profile-art.png','player-default.png','leader-flames.png'})
    if name not in require_names:raise Http404
    path=settings.BASE_DIR/'static'/name
    response=FileResponse(path.open('rb'),content_type='image/png' if name in {'event-ink.png','matchday-landscape.png','team-profile-art.png','player-default.png','leader-flames.png'} else 'text/javascript' if name.endswith('.js') else 'text/css')
